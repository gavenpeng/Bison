...
//          }
          ...
          //    region.batchMutate(mutationsWithLocks.toArray(new Pair[]{}));
            ...
            OperationStatus[] codes = region.put(mutationsWithLocks
        		                 .toArray(new Pair[] {}));
          ...
          //  OperationStatus code = codes[i];
            for (int i = 0; i < codes.length; i++) {
             ...
             List<Mutation> mutationAssistant = new ArrayList<Mutation>();
             for (Action<R> a : mutations) {
               Mutation m = (Mutation) a.getAction();
               Integer lock = getLockFromId(m.getLockId());
               rowsToLockId.put(a.getAction().getRow(), lock);
               mutationAssistant.add(m);
             }
             region.mutateRowsWithLocks(mutationAssistant, rowsToLockId);
             for (Action<R> a : mutations) {
