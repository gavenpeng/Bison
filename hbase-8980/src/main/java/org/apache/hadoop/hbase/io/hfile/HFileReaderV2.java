...
public class HFileReaderV2 extends AbstractHFileReader {
  ...
  public HFileReaderV2(Path path, FixedFileTrailer trailer,
    ...
    //load-on-open data 部分报过 root-data-index，meta-index，file info，Bloom filters（options）
    //trailer.getLoadOnOpenDataOffset() 是load on open data部分在整个HFile的偏移量。
    //fileSize - trailer.getTrailerSize() 是load on open data部分在HFile的截止位置.
    ...
    // 加载load-on-open部分中其他的块，比如bloom filter meta 块。并不是实际的数据lock
    HFileBlock b;
  ...
  public HFileBlock readBlock(long dataBlockOffset, long onDiskBlockSize,
    ...
    try {
      while (true) {
    	...
    	//
    	//读块时先从cache读，这个时候不需要加lock，如果cache没有读到，则开始加lock，
    	//再从cache读一遍，以防止其他的线程读完该block，加入到cache。
    	//加lock是通过块的偏移量为准。如果该偏移量有线程在读，则其他线程等待读线程读完。否则一直block住。
        if (useLock) {
      ...
      if (lockEntry != null) {
    	 //释放该block的lock，如果该block有线程等待，则通知等待线程，
    	 //否则把该block的lock entity从map中remove掉。
  ...
  protected static class ScannerV2 extends AbstractScannerV2 {
    ...
    public boolean next() throws IOException {
      ...
      try {
    	//设置下一个KeyValue的offset在buffer的position
    ...
    private final void readKeyValueLen() {
      //标记下当前buffer的position的位置
      blockBuffer.mark();
      //下一个KeyValue的key的长度
      currKeyLen = blockBuffer.getInt();
      //下一个KeyValue的value的长度
      currValueLen = blockBuffer.getInt();
      //重新设置buffer的position的位置为上面mark时的位置。
      blockBuffer.reset();
      if (this.reader.shouldIncludeMemstoreTS()) {
        try {
