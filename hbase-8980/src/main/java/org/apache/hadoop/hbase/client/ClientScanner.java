...
public class ClientScanner extends AbstractClientScanner {
    ...
    protected boolean checkScanStopRow(final byte [] endKey) {
    	if(this.scan.getAssistantScan()!=null){
    		//如果是索引scan，则需要对localStartKey进行重新计算，因为主scan的localStartKey为null，直接用做localStartKey
    		//，导致region定位不准，则会从第一个region开始，直到region的stopkey大于scan的stopKey，所以需要重新计算
    		//因为一般索引的rowkey和主表的rowkey会有相同的前缀，则我们以相同的前缀作为startKey。这样能更好的定位到region。
    		byte[] assistantStopKey = this.scan.getAssistantScanRowPrefix();
    		if(assistantStopKey == null){
	    		String assistantStopRowKey = Bytes.toString(this.scan.getAssistantScan().getStopRow());
	    		if(assistantStopRowKey!=null){
		    		String[] rowMute = assistantStopRowKey.split("#");
		    		if(rowMute!=null&&rowMute.length>=3){
			    		String assistantLocalStopRow = rowMute[0]+"#"+rowMute[1];
			    		assistantStopKey = Bytes.toBytes(assistantLocalStopRow);
		    		}
	    		}
    		}
    		if(assistantStopKey!=null){
	    		String regionStopRow = Bytes.toString(endKey);
	    		String[] regionStopMute = regionStopRow.split("#");
	    		byte[] regionStopPrex = Bytes.toBytes(regionStopMute[0]+"#"+regionStopMute[1]);
	    		int cmp = Bytes.compareTo(assistantStopKey, 0, assistantStopKey.length,
	    				regionStopPrex, 0, regionStopPrex.length);
		        if (cmp <0) {
    ...
    throws IOException {
      ...
      if (this.currentRegion != null) {
        ...
        if (LOG.isDebugEnabled()) {
          ...
          LOG.debug("Finished with region " + this.currentRegion+",scan time:"+(etime-stime));
        }
      } else {
    	if(this.scan.getAssistantScan()!=null){
    		//如果是索引scan，则需要对localStartKey进行重新计算，因为主scan的localStartKey为null，直接用做localStartKey
    		//，导致region定位不准，则会从第一个region开始，直到region的stopkey大于scan的stopKey，所以需要重新计算
    		//因为一般索引的rowkey和主表的rowkey会有相同的前缀，则我们以相同的前缀作为startKey。这样能更好的定位到region。
    		if(this.scan.getAssistantScanRowPrefix()==null){
	    		String assistantStartRowKey = Bytes.toString(this.scan.getAssistantScan().getStartRow());
	    		if(assistantStartRowKey!=null){
		    		String[] rowMute = assistantStartRowKey.split("#");
		    		if(rowMute!=null&&rowMute.length>=3){
			    		String assistantLocalStartRow = rowMute[0]+"#"+rowMute[1];
			    		localStartKey = Bytes.toBytes(assistantLocalStartRow);
		    		}else{
    			...
    			localStartKey = this.scan.getAssistantScanRowPrefix();
    		}
